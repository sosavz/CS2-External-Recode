...
namespace features::combat {
	...
	shared::context shared::ctx( ) const
	{
		std::shared_lock lock( this->m_ctx_mutex );
		return this->m_ctx;
	}

	shared::penetration shared::pen( ) const
	{
		std::shared_lock lock( this->m_ctx_mutex );
		return this->m_pen;
	}
	...
	void shared::tick( )
	{
		...
		penetration fresh_pen{};
		fresh_pen.prepare( ctx.weapon_vdata, ctx.weapon );

		std::unique_lock lock( this->m_ctx_mutex );
		this->m_ctx = ctx;
		this->m_pen = fresh_pen;
	}
	...
	float shared::calculate_hitchance( const math::vector3& eye_pos, const math::vector3& aim_angle, const systems::collector::player& target, const systems::bones::data& bones ) const
	{
		...
		const auto range = this->pen( ).get_weapon_data( ).range;
		if ( range <= 0.0f )
		{
			return 0.0f;
		}
	...
	math::vector3 shared::extrapolate_stop( const math::vector3& pos ) const
	{
		...
		const auto local_ctx = this->ctx( );
		const auto current_time = local_ctx.current_time;
		...
		if ( use_weapon_speed && local_ctx.weapon_vdata )
		{
			const auto wpn_speed = g::memory.read<float>( local_ctx.weapon_vdata + SCHEMA( "CCSWeaponBaseVData", "m_flMaxSpeed"_hash ) );
			weapon_speed_ratio = std::fminf( 1.0f, wpn_speed / 250.0f );

			if ( local_ctx.weapon )
			{
				const auto zoom = g::memory.read<std::int32_t>( local_ctx.weapon + SCHEMA( "C_CSWeaponBaseGun", "m_zoomLevel"_hash ) );
				const auto zoom_count = g::memory.read<std::int32_t>( local_ctx.weapon_vdata + SCHEMA( "CCSWeaponBaseVData", "m_nZoomLevels"_hash ) );
	...
	{
		const auto local_ctx = this->ctx( );
		if ( local_ctx.weapon_type != cstypes::sniper )
		{
			return true;
		}
