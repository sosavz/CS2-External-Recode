...
bool memory::initialize( std::wstring_view process_name )
{
	...
	this->m_handle = ::OpenProcess( PROCESS_VM_READ | PROCESS_QUERY_INFORMATION | SYNCHRONIZE, FALSE, this->m_id );
	if ( !this->m_handle )
	{
		return false;
	}
...
bool memory::process_alive( ) const
{
	if ( !this->m_handle )
	{
		return false;
	...
	DWORD exit_code{};
	if ( !::GetExitCodeProcess( static_cast< HANDLE >( this->m_handle ), &exit_code ) )
	{
		return false;
	}
