...
namespace zui {
	...
	bool checkbox( std::string_view label, bool& v )
	{
		...
		const auto border_col = hovered ? lighten( style.checkbox_border, 1.2f ) : style.checkbox_border;
		...
		if ( v )
		{
			...
			const auto x0 = abs.x + 3.0f;
			const auto y0 = abs.y + abs.h * 0.55f;
			const auto x1 = abs.x + abs.w * 0.45f;
			const auto y1 = abs.y + abs.h - 3.5f;
			const auto x2 = abs.x + abs.w - 3.0f;
			const auto y2 = abs.y + 3.5f;

			active_draw_list( ).add_line( x0, y0, x1, y1, check_col, 2.0f );
			active_draw_list( ).add_line( x1, y1, x2, y2, check_col, 2.0f );
		}
	...
	namespace
	{
		...
		bool slider_impl( std::string_view label, T& v, T v_min, T v_max, std::string_view format )
		{
			...
			const auto hover_anim = ( hovered || is_active ) ? 1.0f : 0.0f;
			const auto range = static_cast< float >( v_max - v_min );
			const auto normalized_pos = ( static_cast< float >( v ) - static_cast< float >( v_min ) ) / range;
			...
			const auto value_col = lerp( style.text, style.slider_fill, hover_anim * 0.35f );
			active_draw_list( ).add_text( abs.x + slider_width - value_w, abs.y, value_buf, zdraw::get_font( ), value_col );

			const auto track_bg = style.slider_bg;
			active_draw_list( ).add_rect_filled( track_rect.x, track_rect.y, track_rect.w, track_rect.h, track_bg );

			const auto border_col = hovered ? lighten( style.slider_border, 1.15f ) : style.slider_border;
			...
			if ( fill_w > 0.5f )
			{
				...
				active_draw_list( ).add_rect_filled( fill_x, fill_y, fill_w, fill_h, style.slider_fill );
			}

			const auto knob_bg = hovered ? lighten( style.slider_fill, 1.1f ) : style.slider_fill;
			active_draw_list( ).add_rect_filled( knob_x, knob_y, knob_width, knob_height, knob_bg );
			active_draw_list( ).add_rect( knob_x, knob_y, knob_width, knob_height, style.slider_border );
	...
	bool text_input( std::string_view label, std::string& text, std::size_t max_length, std::string_view hint )
	{
		...
		if ( is_active )
		{
			...
			auto insert_char = [ & ]( char ch )
				{
					...
					const auto has_sel = state.selection_start != state.selection_end;
					const auto min_sel = std::min( state.selection_start, state.selection_end );
					const auto max_sel = std::max( state.selection_start, state.selection_end );

					if ( has_sel )
					{
						text.erase( min_sel, max_sel - min_sel );
						state.cursor_pos = min_sel;
					}
			...
			for ( int vk = 'A'; vk <= 'Z'; ++vk )
			{
				if ( ::GetAsyncKeyState( vk ) & 1 )
				{
					const auto base = static_cast<char>( vk );
					const auto ch = shift_held ? base : static_cast<char>( std::tolower( static_cast<unsigned char>( base ) ) );
					insert_char( ch );
				}
			}
			...
			for ( int vk = '0'; vk <= '9'; ++vk )
			{
				if ( ::GetAsyncKeyState( vk ) & 1 )
				{
					insert_char( static_cast<char>( vk ) );
				}
			}
