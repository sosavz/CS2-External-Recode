...
namespace zdraw {
	...
	void draw_list::add_text( float x, float y, std::string_view text, const font* f, rgba color, text_style style )
	{
		...
		const auto faux_bold = ( f == get_font( ) );
		...
		for ( char c : text )
		{
			...
			if ( char_w > 0.0f && char_h > 0.0f )
			{
				...
				if ( faux_bold )
				{
					constexpr auto bold_offset{ 0.42f };
					const auto bold_vtx_base{ static_cast< std::uint32_t >( this->m_vertices.size( ) ) };

					this->push_vertex( char_x + bold_offset, char_y, glyph.m_uv_x0, glyph.m_uv_y0, color );
					this->push_vertex( char_x + char_w + bold_offset, char_y, glyph.m_uv_x1, glyph.m_uv_y0, color );
					this->push_vertex( char_x + char_w + bold_offset, char_y + char_h, glyph.m_uv_x1, glyph.m_uv_y1, color );
					this->push_vertex( char_x + bold_offset, char_y + char_h, glyph.m_uv_x0, glyph.m_uv_y1, color );

					auto bold_idx{ this->m_indices.allocate( 6 ) };
					bold_idx[ 0 ] = bold_vtx_base; bold_idx[ 1 ] = bold_vtx_base + 1; bold_idx[ 2 ] = bold_vtx_base + 2;
					bold_idx[ 3 ] = bold_vtx_base; bold_idx[ 4 ] = bold_vtx_base + 2; bold_idx[ 5 ] = bold_vtx_base + 3;
